 ...
 * @since 2.0.0
 */
public class HtmlMultipleSelectorWithAttributeMarkupHandlerTest extends TestCase {

    private static final String RESOURCES_FOLDER = "htmlmultipleselectorwithattribute/";

    public HtmlMultipleSelectorWithAttributeMarkupHandlerTest() {
        super();
    }
    ...
    public void test() throws Exception {
        ...
        for (int i = 0; i < testFiles.size(); i++) {
            ...
            final String[] selectorLevels = StringUtils.split(blockSelector,":");

            check(parser, testFile.getName(), testFileContents, resultFileContents, selectorLevels, referenceResolver);
    ...
    private static void check(
            final IMarkupParser parser, final String templateName, final String input, final String output, final String[] selectorLevels,
        ...
        IMarkupHandler handler = attributeMarkingHandler;

        String[] selectors = StringUtils.split(selectorLevels[selectorLevels.length - 1],",");
        handler = new NodeSelectorMarkupHandler(handler, new DiscardMarkupHandler(), selectors, referenceResolver);
        if (selectorLevels.length > 1) {
            for (int i = selectorLevels.length - 2; i >= 0; i--) {
                selectors = StringUtils.split(selectorLevels[i],",");
                handler = new BlockSelectorMarkupHandler(handler, new DiscardMarkupHandler(), selectors, referenceResolver);
            }
        }
